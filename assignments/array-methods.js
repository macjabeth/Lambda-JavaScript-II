...
const allCaps = runners.map(({ first_name }) => first_name.toUpperCase());
console.log(allCaps);
...
// Problem 1 - Highest Donating Companies
const highestDonaters = runners.sort((a, b) => b.donation - a.donation).map(({ company_name, donation }) => [company_name, donation]);
console.log(highestDonaters);

// Problem 2 - Marketing Emails >:]
const emails = runners.map(({ email }) => email);
console.log(emails);

// Problem 3 - Grouping by shirt_size
const groupedRunners = runners.reduce((acc, cur) => {
  const { shirt_size } = cur;
  if (!acc[shirt_size]) acc[shirt_size] = [];
  acc[shirt_size].push(cur);
  return acc;
}, {});
