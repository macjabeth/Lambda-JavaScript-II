...
let highestDonaters = runners.sort((a, b) => b.donation - a.donation).map(({ company_name, donation }) => [company_name, donation]);
console.log(highestDonaters);

// Problem 2 - Marketing Emails >:]
let emails = runners.map(({ email }) => email);
console.log(emails);

// Problem 3 - Grouping by shirt_size
let groupedRunners = runners.reduce((acc, cur) => {
